...
## Aggregate
Aggregate data is deerived from more than one row at a time. The count function is an example of an aggregate function.

```
SELECT Region, COUNT(*) as Count
  FROM Country;
  GROUP BY Region
  HAVING Region >= 3 -- similar to where but works on aggregate data, where works on non-aggregate data
  ORDER BY Count DESC, Region;

SELECT COUNT(Name) FROM Country; -- count non-null values of column
SELECT Region, AVG(Population) FROM Country; -- counts average value, for non-null value columns
SELECT Region, MIN(Population) FROM Country; -- counts min
SELECT Region, SUM(Population) FROM Country; -- counts the sum

/*
Can use distinct used with any of the other aggregate functions
*/
SELECT COUNT(DISTINCT HeadOfState) FROM Country; -- counts only distinct instances

```

## Transactions

A transaction is a group of operations that are handled as one unit, if any of these operations fail, the entire group of operations also fail, afterward the database is then restored to a state before the operations started. In some instances, transactions can also drastically improve performance as well as ensure database integrity. For instance, instead of doing a bunch of small writes to a database, you can bulk them up into a single transaction. Each individual write adds overhead, which over time, would slow down your system, however grouping these writes into a transaction will significantly reduce the overhead to writing to physical media.

A demonstration of a transaction is bank transactions. For instance, suppose you decided to deposit money into your account, and then withdraw a small portion of your account. If these actions do not occur in a specific order, they transaction should fail, otherwise we can get devasting cases. If your current balance is $0.00 and you decide to deposit $100.00, and right after you need $20.00 to withdraw, if the deposit never successfully occurs however the withdraw occurs, you'll be attempting to withdraw $20.00 from your bank, when you have insufficient funds. This is why transactions are essential. Either the group of operations occur, or none of them do.


```
BEGIN TRANSACTION
INSERT INTO Sales (id, quantity, price) VALUES (1,5,200);
UPDATE Inventory SET available = (available - 5) WHERE id = 1;
END TRANSACTION

BEGIN TRANSACTION
INSERT INTO Sales (id, quantity, price) VALUES (1,5,200);
ROLLBACK;  -- roll back insertion

/*
Test statement with and without transactions but copy
and paste insertion a couple hundred times and test the miliseconds passed between  
the two.
*/

BEGIN TRANSACTION
INSERT INTO Sales (DESCRIPTION) VALUES ('HELLO');
INSERT INTO Sales (DESCRIPTION) VALUES ('HELLO');
INSERT INTO Sales (DESCRIPTION) VALUES ('HELLO');
INSERT INTO Sales (DESCRIPTION) VALUES ('HELLO');
INSERT INTO Sales (DESCRIPTION) VALUES ('HELLO');
...
```
CREATE TRIGGER Sale AFTER INSERT ON Sale
  BEGIN
    UPDATE Sale SET last_order = NEW.id WHERE Customer.id = NEW.customer_id
  END;

/*
You can also use triggers to prevent updates
*/

CREATE TRIGGER Sale BEFORE UPDATE ON Sale
  BEING
    SELECT RAISE(ROLLBACK, 'cannot update') FROM Sale
      WHERE id = NEW.id and blocked = 1 
  END;
```
